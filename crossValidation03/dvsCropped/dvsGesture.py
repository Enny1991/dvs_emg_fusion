...
class handDvsDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        inputSpikes = snn.io.event(
                        ...
                        ).toSpikeTensor(torch.zeros((2,40,40,self.nTimeBins)),
...
class Network(torch.nn.Module):
    def __init__(self, netParams):
        ...
        self.conv1 = torch.nn.utils.weight_norm(slayer.conv( 2,  8, 3, padding=1, weightScale=20), name='weight')
        self.conv2 = torch.nn.utils.weight_norm(slayer.conv( 8, 16, 3, padding=1, weightScale=100), name='weight')
        self.conv3 = torch.nn.utils.weight_norm(slayer.conv(16, 32, 3, padding=1, weightScale=100), name='weight')
        self.fc1   = torch.nn.utils.weight_norm(slayer.dense((10*10*32), 512), name='weight')
...
def genLoihiParams(net):
    ...
    loihiWeights = lambda x: snn.quantize.apply(x, 2).cpu().data.numpy()
    ...
    convWeights.append( loihiWeights(net.conv3.weight) )
...
if __name__ == '__main__':
    ...
    trainingSet = handDvsDataset(samples     =np.loadtxt('../train.txt').astype(int),
                                 samplingTime=netParams['simulation']['Ts'],
                                 sampleLength=netParams['simulation']['tSample'],
                                 augment     =True)
    trainLoader = DataLoader(dataset=trainingSet, batch_size=12, shuffle=True, num_workers=1)

    testingSet = handDvsDataset(samples     =np.loadtxt('../test.txt').astype(int),
                                samplingTime=netParams['simulation']['Ts'],
                                sampleLength=netParams['simulation']['tSample'])
    testLoader = DataLoader(dataset=testingSet, batch_size=12, shuffle=True, num_workers=1)
    ...
    #     snn.io.showTD(snn.io.spikeArrayToEvent(input.reshape((2, 40, 40, -1)).cpu().data.numpy()), repeat=True)
        
    for epoch in range(lastEpoch, 1000):
        tSt = datetime.now()
